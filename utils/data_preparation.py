...
def paths_from_region(path_to_data):
  ...
  for region in regions:
    ...
    for speaker in speakers:
      ...
      names = [speaker_path + "/" + f for f in set([file.split(".")[0] for file in speaches_path_full])]
      _paths += names
  return _paths
...
def read_phn(f, temp_mfcc, phonem_dict):
  # Read PHN files
  temp_phones = pd.read_csv(f, delimiter=" ", header=None,  names=['start', 'end', 'phone']) 
  ...
  if phn_len_mill < temp_mfcc.shape[1]:
    ...
    phones = np.zeros((len(phonem_dict.keys()), phn_len_mill), dtype=int)
